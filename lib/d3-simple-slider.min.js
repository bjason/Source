...
! function (t, e) {
    ...
    function h(t, h) {
        ...
        function I(r) {
            T = r.selection ? r.selection() : r, h = h ? h.range([e.min(h.range()), e.min(h.range()) + (t === o || t === f ? y : A)]) : (h = x[0] instanceof Date ? i.scaleTime() : i.scaleLinear()).domain(x).range([0, t === o || t === f ? y : A]).clamp(!0), j = i.scaleLinear().range(h.range()).domain(h.range()).clamp(!0), v = v.map(function (t) {
                return i.scaleLinear().range(x).domain(x).clamp(!0)(t)
